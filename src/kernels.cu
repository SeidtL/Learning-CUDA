...
void trace_kernel(
  ...
  const T warp_sum = warp_trace_reduce(sum);
  ...
  if (tid < warp_size) {
    ...
    block_sum = warp_trace_reduce(block_sum);
    if (tid == 0) {
...
template <typename T, typename U, size_t max_head_dim=64, uint32_t Bc=32, uint32_t Tr=1>
__global__
void attention_kernel(
  ...
  static_assert(Bc == warp_size, "unsupport");
    ...
    for (int j = 0; j < Tc; ++j) {
      ...
      if (q_valid && k_block_valid) {
        ...
        for (uint32_t offset = warp_size >> 1; offset > 0; offset >>= 1) {
          m_new = max(m_new, __shfl_xor_sync(0xFFFFFFFF, m_new, offset));
        }
...
template <typename T, typename U, uint32_t Bc, uint32_t Tr, typename ...Args> 
void attention_dispatch(size_t head_dim, dim3 grid, dim3 block, size_t smem_size, Args&&... args) {
  if (head_dim <= 32) {
    attention_kernel<T, U, 32, Bc, Tr><<<grid, block, smem_size>>>(std::forward<Args>(args)...);
  } else if (head_dim <= 64) {
    attention_kernel<T, U, 64, Bc, Tr><<<grid, block, smem_size>>>(std::forward<Args>(args)...);
  } else if (head_dim <= 128) {
    attention_kernel<T, U, 128, Bc, Tr><<<grid, block, smem_size>>>(std::forward<Args>(args)...);
  } else {
    S_ASSERT(false);
  }
...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
  ...
  RUNTIME_CHECK(cudaMalloc(&sum, sizeof(T)));
  RUNTIME_CHECK(cudaMalloc(&inp, sizeof(T) * n));
  RUNTIME_CHECK(cudaMemset(sum, 0, sizeof(T)));

  std::vector<T> buf;
  buf.reserve(n);
  for (int i = 0; i < n; ++i) {
    buf.emplace_back(h_input[i * cols + i]);
  }
  RUNTIME_CHECK(cudaMemcpy(inp, buf.data(), sizeof(T) * n, cudaMemcpyHostToDevice));

  trace_kernel<T><<<grid, block, smem_size>>>(
    inp, 
    sum, 
    n
  );
  ...
  RUNTIME_CHECK(cudaMemcpy(&result, sum, sizeof(T), cudaMemcpyDeviceToHost));
  RUNTIME_CHECK(cudaFree(inp));
  RUNTIME_CHECK(cudaFree(sum));
  return result;
}
